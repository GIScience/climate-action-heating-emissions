...
from heating_emissions.components.temporal_downscale.demand_ninja.demand_ninja.util import (
    get_cdd,
    get_hdd,
    smooth_temperature,
)

DIURNAL_PROFILES = pd.read_csv(os.path.join(os.path.dirname(__file__), 'diurnal_profiles.csv'), index_col=0)
...
def _bait(
    ...
    setpoint_H = (1.1 + 0.06 * T).rpow(np.e) / 1000  # g water per kg air # note: modified by Gefei: g/kg -> kg/kg
...
def _energy_demand_from_bait(
    ...
    # Sum total demand
    output['total_demand'] = base_power + output['heating_demand'] + output['cooling_demand']
...
def demand(
    ...
    daily_bait.index = pd.date_range(
        daily_bait.index[0] + pd.Timedelta('12h'),  # note: modified by Gefei: 12H -> 12h
